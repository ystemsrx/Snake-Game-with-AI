...
#define OBSTACLE_REFRESH_INTERVAL 3
...
void drawObstacles(Point obstacles[], int obstacleCount) {
...
void eraseObstacles(Point obstacles[], int obstacleCount) {
    for (int i = 0; i < obstacleCount; i++) {
        gotoxy(obstacles[i].x, obstacles[i].y);
        printf(" ");
    }
...
void generateObstacles(Point obstacles[], int count, Snake *snake) {
...
int checkObstacleCollision(Snake *snake, Point obstacles[], int obstacleCount) {
    for (int i = 0; i < obstacleCount; i++) {
...
void gameOver() {
    ...
    writeToDocument("The player failed due to hitting an obstacle.");
...
int main(int argc, char *argv[]) {
    ...
    writeToDocument("The player starts the game, gamemode: Master.");
    ...
    clock_t lastObstacleTime = clock();
    clock_t trimTime = 0;
    clock_t currentTime;
    ...
    initialize(&snake);
    generateFood(&food);
    generateObstacles(obstacles, OBSTACLE_COUNT, &snake);

    drawMap();
    drawObstacles(obstacles, OBSTACLE_COUNT);
    drawSnake(&snake);
    drawFood(&food);
    ...
    while (1) {
        ...
        currentTime = clock();
        gotoxy(0, HEIGHT);
        printf("Current Score: %d", snake.length - INIT_LENGTH);
        gotoxy(20, HEIGHT);
        if (bonusActive) {
        ...
        // 随机移动障碍物
        if ((currentTime - lastObstacleTime) / CLOCKS_PER_SEC >= OBSTACLE_REFRESH_INTERVAL) {
            eraseObstacles(obstacles, OBSTACLE_COUNT);
            generateObstacles(obstacles, OBSTACLE_COUNT, &snake);
            drawObstacles(obstacles, OBSTACLE_COUNT);
        ...
        if (_kbhit()) {
            ...
            switch (key) {
                ...
                case 'r': return main(argc, argv); // 重新开始游戏
            }
        }
        ...
        if (checkCollision(&snake, &food)) {
            ...
            growSnake(&snake, 1);
            char message[50];
            sprintf(message, "The player has ate a food and got 1 point (%d/30)", snake.length - INIT_LENGTH);
            writeToDocument(message);
        } else if (bonusActive && checkBonusCollision(&snake, &bonusFood)) {
            growSnake(&snake, 5);
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    gotoxy(bonusFood.x + i, bonusFood.y + j);
            ...
            lastBonusTime = currentTime;
            char message[50];
            sprintf(message, "The player has ate a bonus food and got 5 points (%d/30)", snake.length - INIT_LENGTH);
            writeToDocument(message);
        } else if (snake.body[0].x == 0 || snake.body[0].x == WIDTH - 1 || snake.body[0].y == 0 || snake.body[0].y == HEIGHT - 1 || checkSelfCollision(&snake) != -1 || checkObstacleCollision(&snake, obstacles, OBSTACLE_COUNT)) {
            gameOver();
            char key;
            while (key != 'r') {
                if (_kbhit()) {
            ...
            return main(argc, argv); // 重新开始游戏
        }

        if (bonusActive && (currentTime - lastBonusTime) / CLOCKS_PER_SEC >= 8) {
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    gotoxy(bonusFood.x + i, bonusFood.y + j);
                    printf(" ");
