...
int main(int argc, char *argv[]) {
    ...
    writeToDocument("The player starts the game, gamemode: Normal.");
    ...
    while (1) {
        ...
        // 检查碰撞
        if (snake.body[0].x == 0 || snake.body[0].x == WIDTH - 1 || snake.body[0].y == 0 || snake.body[0].y == HEIGHT - 1 || checkSelfCollision(&snake) != -1) {
            gameOver();
            char key;
            while (key != 'r') {
                if (_kbhit()) {
